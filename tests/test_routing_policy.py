...
def make_policy(
    *,
    ...
    provider_registry = ProviderRegistry(
        registered_providers or (MockProviderAdapter(),)
    )
...
def test_missing_provider_raises_provider_not_found() -> None:
    with pytest.raises(ProviderNotFoundError) as exc_info:
        make_policy(providers=("missing",)).route(
            RoutingRequest(requested_model="public-model")
        )
...
def test_route_returns_routing_decision() -> None:
    assert isinstance(
        make_policy().route(RoutingRequest(requested_model="public-model")),
        RoutingDecision,
    )
